...
def add_to_local_manifest(path, name, remote, branch=None):
    ...
    if (remote == "cd"):
        if (branch == None):
            branch = "c6"
        if not (name.find("CandyDevices/") == 0):
            name = "CandyDevices/" + name
...
def get_from_github(device):
        print("Going to fetch %s from CandyDevices github" % device)
        try:
        ...
        githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:CandyDevices+in:name+fork:true" % device)
        if githubauth:
        ...
        for res in result['items']:
            if (res['name'].startswith("device_") and res['name'].endswith("_%s" % device)):
                ...
                if add_to_local_manifest(devicepath, res['full_name'], "candy"):
                    reposync(res['full_name'])
                break

def checkdeps(repo_path):
    cmdeps = glob.glob(repo_path + "/cm.dependencies")
    candydeps = glob.glob(repo_path + "/candy.dependencies")
    if ((len(cmdeps) + len(candydeps)) < 1):
        ...
        if (len(candydeps) > 0):
            ran_checkdeps_on.append("HAS_CANDY_DEPS:\t" + repo_path)
            candydeps = candydeps[0]
            candydeps = open(candydeps, 'r')
            candydeps = json.loads(candydeps.read())
            for dep in candydeps:
                try:
                    branch = dep['branch']
                except:
                    branch = None
